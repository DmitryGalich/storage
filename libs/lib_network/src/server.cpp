...
namespace network
{
    ...
    while (communication_data_.is_need_cycling_.load())
    {
      ...
      for (int event_i = 0; event_i < kEventsCount; event_i++)
      {
        ...
        else
        {
          ...
          {
            LOG_ERROR(std::string(exception.what()));
  ...
  void Server::Implementation::process_new_connection()
  {
    ...
    LOG_INFO("processing new connection...");
    ...
    LOG_INFO("new connection processed");
  }
      ...
      const descriptor &client_fd)
  {
    ...
    LOG_INFO("client_fd(" + std::to_string(client_fd) +
                 ") read message: " + buffer,
             __PRETTY_FUNCTION__);
    ...
    LOG_INFO("written to client_fd(" + std::to_string(client_fd) +
  ...
  {
    LOG_INFO("closing connection(" + std::to_string(client_fd) + ")...",
             __PRETTY_FUNCTION__);
    ...
    LOG_INFO("connection (" + std::to_string(client_fd) + ") closed",
             __PRETTY_FUNCTION__);
  }
